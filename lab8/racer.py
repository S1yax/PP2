...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = car1
        #враг не появится слишком близко к игроку 
        while True:
            self.rect = self.image.get_rect(center=(random.randint(40, SCREEN_WIDTH - 40), 0))
            if abs(self.rect.centerx - P1.rect.centerx) > 50: #Проверяется расстояние между игроком и желтой машинкой
                break
    ...
    def move(self):
        ...
        if self.rect.top > SCREEN_HEIGHT:
            ...
            self.rect.top = 0 #перемещаем врага обратно вверх после того как он достигает края экрана
            self.rect.centerx = random.randint(40, SCREEN_WIDTH - 40) #новое рандомное появление
class Player(pygame.sprite.Sprite):
    ...
    def move(self):
        pressed_keys = pygame.key.get_pressed() #управление машиной через клавиатуру и список всех нажатых клави
...
INC_SPEED = pygame.USEREVENT + 1 #ускорение 
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, enemies): #Если P1 касается enemies
